...
func (t systemSetType) Equals(ctx context.Context, otherType sql.Type) bool {
	if ot, ok := otherType.(systemSetType); ok {
		return t.varName == ot.varName && t.SetType.Equals(ctx, ot.SetType)
	}
	return false
}
...
func (t systemSetType) DecodeValue(val string) (interface{}, error) {
	// context will never be dereferenced, it's safe to pass in nil
	outVal, _, err := t.Convert(nil, val)
	if err != nil {
